...
@pytest.mark.sanity
@pytest.mark.init_md(True)
def test_purge_all_files_in_removed_state(working_dir, rm_cmd, md_manager, session):
    filepaths = [
    ...
    subprocess.check_output([*rm_cmd, filepaths[1]])
    subprocess.check_output([*rm_cmd, filepaths[2]])
    subprocess.check_output([*rm_cmd, "--purge"])
    session.expire_all()
